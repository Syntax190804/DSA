...
if(root->lbit==0)
{
temp->left=root->left; //root left is pointing to head so
//now temp left will point to head
temp->right=root; // temp right to root
root->left=temp; // root left to temp
root->lbit=1;
}
else
get_insert(root->left,temp); //for next insertions root
//left i.e. temp will become root ;
 }
